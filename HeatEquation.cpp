...
// only small matrices will be distributed across all nodes.
    ...
    if(mynode==0)
    {
        ...
        for(i=0;i<L+1;i++) times.push_back(i*timestep + delta*timestep*(((double) rand() / (RAND_MAX)) - 0.5));
        times[0] = 0; times[L] = 1;
        // Calculate timesteps
        for(i=0;i<L;i++)timesteps.push_back(times[i+1]-times[i]);
        // Calculate perturbations
        for(i=0;i<L+1;i++)perts.push_back(times[i]-i*timestep);
...
//
        ...
        for(int i=0;i<L;i++)
        {
            ...
            MultiplyTriDiagByConst(N,(1.0-D[i*L+i]),temp);
        ...
        // packaging into monolithic system.
...
//
        ...
        for(int i=0;i<L;i++)
        {
            ...
            MultiplyTriDiagByConst(N,timesteps[i],temp);
...
//  
    ...
    //  For tol = 10^{-5} we get a fixed iteration count of 2 for suff. large n,l
...
//==========================================  ARNODLI BEGIN
            MultiplyByHeatSystem(mynode,totalnodes,N,L,Ablocks,massContig,v[i],temp);
